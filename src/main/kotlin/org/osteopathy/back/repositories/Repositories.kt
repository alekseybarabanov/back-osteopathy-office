...
interface PatientRepository : CrudRepository<Patient, Long> {
    ...
    @Query("SELECT t FROM Patient t where t.tenant = ?3 AND " +
            "((t.firstName like '%' || ?1 || '%' OR t.lastName like '%' || ?1 || '%' OR t.middleName like '%' || ?1 || '%') AND " +
            "(t.firstName like '%' || ?2 || '%' OR t.lastName like '%' || ?2 || '%' OR t.middleName like '%' || ?2 || '%'))")
    fun findByNameWords(word1: String, word2: String, tenant: String?): List<Patient>
}
